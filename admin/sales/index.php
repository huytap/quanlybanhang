				...
				<table class="table table-hover table-striped table-bordered">
					...
					<tbody>
						...
						if ($_settings->userdata('type') == 3) :
							$qry = $conn->query("SELECT * FROM `sale_list` where deleted_flag=0 AND user_id = '{$_settings->userdata('id')}' order by unix_timestamp(date_updated) desc ");
						else :
							$qry = $conn->query("SELECT * FROM `sale_list` where deleted_flag=0 order by date_updated desc ");
						endif;
